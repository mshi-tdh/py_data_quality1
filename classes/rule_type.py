...
COUNT_SQL = Template("SELECT COUNT(*) FROM")
...
def to_sql_custom_sql_expr(params: dict) -> Template:
    ...
    logger.info(f"custom_sql_expr:{custom_sql_expr}")
    ...
    logger.info(f"custom_sql_arguments:{custom_sql_arguments}")
    if custom_sql_arguments:
        for argument in custom_sql_arguments:
            logger.info(f"argument value:{argument},rule_binding_arguments:{params.get('rule_binding_arguments')}")
    ...
    logger.info(f"rule_binding_arguments:{params.get('rule_binding_arguments')}")
...
def to_sql_custom_sql_statement(params: dict) -> Template:
    ...
    print(f"custom_sql_statement={custom_sql_statement}")
    assert_not_none_or_empty(
        custom_sql_statement,
    ...
    if custom_sql_arguments:
        for argument in custom_sql_arguments:
            logger.info(f"argument:{argument},{params.get('rule_binding_arguments')}")
            ...
            logger.info(f"rule_binding_arguments:{params.get('rule_binding_arguments')}")
    if params.get("rule_binding_arguments"):
        logger.info(f"custom_sql_statement_before:{custom_sql_statement}")
        logger.info(f"rule_binding_arguments:{params.get('rule_binding_arguments')}")
        custom_sql_statement = Template(custom_sql_statement).safe_substitute(
            params.get("rule_binding_arguments")
        )
        logger.info(f"custom_sql_statement_after:{custom_sql_statement}")
        #custom_sql_statement = Template(custom_sql_statement).safe_substitute({"tgt_snapshot_value":"202207"})
        #logger.info(f"custom_sql_statement_input_update:{custom_sql_statement}")
...
class RuleType(str, Enum):
    ...
    ENTITY_LEVEL = "RT_ENTITY_LEVEL"
    CUSTOM_SQL_STATEMENT = "RT_AGGREGATE_LEVEL"
    CUSTOM_SQL_EXPR = "RT_ATTRIBUTE_LEVEL"
    ...
    def to_sql(self: RuleType, params: dict | None) -> Template:
        ...
        elif self == RuleType.ENTITY_LEVEL:
            return to_sql_custom_sql_statement(params)
        else:
