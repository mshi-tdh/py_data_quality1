...
from utils import get_keys_from_dict_and_assert_oneof

import classes.dq_configs_cache as dq_configs_cache
        ...
        for rule in self.rule_ids:
            if type(rule) == dict:
                if len(rule) > 1:
                    ...
                    logger.info(f"arguments:rule_id:{rule_id},{arguments},{rule}")
            else:
                rule_id = rule
                arguments = None
    ...
    def resolve_all_configs_to_dict(
        ...
        try:
            ...
            if self.incremental_time_filter_column_id:
                ...
                logger.info(f"incremental_time_filter_column_config:{incremental_time_filter_column_config}")
                incremental_time_filter_column_type: str = (
                    incremental_time_filter_column_config.get_column_type_value()
                )
                logger.info(f"incremental_time_filter_column_type:{incremental_time_filter_column_type}")
            ...
            for rule in self.resolve_rule_config_list(configs_cache):
                for rule_id, rule_config in rule.to_dict().items():
                    ...
                    logger.debug(f"rule:{rule},rule_id:{rule_id},rule_config:{rule_config}")
                    rule_sql_expr=rule_config["rule_sql_expr"]
            ...
            if self.reference_columns_id:
                ...
                include_reference_columns = []
            print(f"rule_binding_id={self.rule_binding_id} and row_filter_configs={row_filter_config.dict_values()} ")
            return dict(
                {
